...
export default function SuperAdminAttendanceDesk() {
  const [activeTab, setActiveTab] = useState("matrix"); // "matrix" | "leaves"
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [adminsRes, leavesRes] = await Promise.all([
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-16">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="admin-lead">Track sub-admin daily login attendance, approve leave requests, and manage acting delegates.</p>
        </div>
      </div>
      ...
      {rejectModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className="bg-[#0b1329] border border-white/15 rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-4">
            <h3 className="text-base font-bold text-white">Decline Leave Request</h3>
            <textarea
              rows={4}
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => handleLeaveDecision(rejectModal.id, "reject", rejectionReason)}
                disabled={updating}
