  ...
  try {
    if (!hasEmailConfig && !isDev) {
      return NextResponse.json(
        { success: false, error: "Email service not configured. Please set EMAIL_USER and EMAIL_PASS in environment." },
        { status: 503 }
      );
    }
    ...
    try {
      await sendMail({
        to: normalizedEmail,
        subject: "Your Account Verification Code - Chakradhar Stream",
      ...
      console.error("[signup-otp] Email dispatch failed:", err?.message || err);
      emailError = err?.message || "SMTP error";
    }

    if (isDev) {
      console.log("\n=======================================================");
      console.log(`🔑 [SIGNUP OTP] Verification code for ${normalizedEmail}:`);
      console.log(`👉 ${otp} 👈`);
      if (!emailSent) {
      ...
      return NextResponse.json({
        ...
        message: emailSent
          ...
          : `Dev Mode: Verification code generated (${otp}). Check console.`,
        devMode: true,
    ...
    if (!emailSent) {
      return NextResponse.json(
        {
          success: false,
          error: `Failed to deliver email: ${emailError || "SMTP delivery error"}. Please check email service configuration.`,
        },
        { status: 500 }
      );
    }
