...
export async function POST(req) {
  ...
  const hasEmailConfig = Boolean(
    ...
    rawPass &&
    !rawPass.toLowerCase().includes("xxxx")
  );

  try {
    if (!hasEmailConfig && !isDev) {
      return NextResponse.json(
        {
          success: false,
          error: "Email service is not configured on production. Please set EMAIL_USER and EMAIL_PASS (16-character Google App Password) in environment.",
        },
        { status: 503 }
      );
    }
    ...
    try {
      await sendMail({
        to: normalizedEmail,
        subject: "Admin OTP Verification - Chakradhar Stream",
      ...
      console.error("[send-otp] Failed to dispatch email via SMTP:", err?.message || err);
      emailError = err?.message || "SMTP error";
    }

    if (isDev) {
      console.log("\n=======================================================");
      console.log(`🔑 [ADMIN LOGIN OTP] Verification code for ${normalizedEmail}:`);
      console.log(`👉 ${otp} 👈`);
      if (!emailSent) {
        console.log(`⚠️ Email was not sent via SMTP: ${emailError || "SMTP not configured"}`);
        console.log(`👉 You can log in using the code above (${otp}) right now!`);
      } else {
        console.log(`✉️ Email successfully dispatched to ${normalizedEmail}!`);
      }
      console.log("=======================================================\n");
      ...
      await logServerEvent("admin_otp_sent", {
        ip,
        email: normalizedEmail,
        mode: emailSent ? "smtp" : "dev_console",
      });
      ...
      return NextResponse.json({
        ...
        message: emailSent
          ...
          : `Dev Mode: Verification code generated (${otp}). Check server console or enter code.`,
        devMode: true,
    ...
    if (!emailSent) {
      return NextResponse.json(
        {
          success: false,
          error: `Failed to deliver email: ${emailError || "SMTP delivery error"}. Please check EMAIL_PASS (must be a 16-character Google App Password).`,
        },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: error?.message || "Server error while processing OTP request" },
      { status: 500 }
    );
  }
}