...
export default function SubAdminAttendancePage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [attRes, leavesRes, adminsRes] = await Promise.all([
  ...
  /* ── 4. Cancel Leave & Resume Duty Mid-Way ── */
  ...
  return (
    <SubAdminAccessGuard moduleKey="dashboard">
      <div className="space-y-8 max-w-6xl mx-auto pb-16">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="admin-title flex items-center gap-2">
              <span>📅</span> Sub-Admin Attendance Calendar
            </h1>
            <p className="admin-lead">Track your monthly presence, apply for leave, and manage acting delegates.</p>
          </div>
        ...
        {/* ACTIVE LEAVE WARNING BANNER WITH DUTY RESUME */}
        {activeApprovedLeave && (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            <div className="bg-[#0b1329] border border-white/15 rounded-3xl p-6 max-w-lg w-full shadow-2xl space-y-5">
              ...
              <form onSubmit={handleSubmitLeave} className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-cyan-300 uppercase mb-1">Leave Type</label>
                  <select
                    value={leaveType}
                    onChange={(e) => setLeaveType(e.target.value)}
                    className="admin-input focus-ring text-xs text-white bg-zinc-900"
                  >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-bold text-cyan-300 uppercase mb-1">Start Date & Time</label>
                    <input
                      type="datetime-local"
                      required
                      value={startDate}
                      onChange={(e) => setStartDate(e.target.value)}
                      className="admin-input focus-ring text-xs text-white"
                    />
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-cyan-300 uppercase mb-1">End Date & Time</label>
                    <input
                      type="datetime-local"
                      required
                      value={endDate}
                ...
                <div>
                  <label className="block text-xs font-bold text-cyan-300 uppercase mb-1">Reason for Leave</label>
                  <textarea
                    rows={3}
                    required
                    value={reason}
                    ...
                    className="admin-input focus-ring text-xs text-white resize-none"
                  />
                </div>

                <div>
                  <label className="block text-xs font-bold text-cyan-300 uppercase mb-1">
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    type="submit"
                    disabled={submitting}
                    className="flex-1 py-2.5 bg-gradient-to-r from-cyan-500 to-blue-600 text-white rounded-xl text-xs font-bold uppercase disabled:opacity-50"
                  >
