      ...
      <div className="space-y-8">
        ...
        <div>
          ...
          <h1 className="admin-title flex items-center gap-2">
            <LockShieldIcon className="w-8 h-8 text-cyan-400" />
            <span>Sub-Admin Control Center</span>
          </h1>
        ...
        {/* PROFILE BANNER */}
        {loading ? (
          <div className="p-6 rounded-3xl bg-white/5 border border-white/10 animate-pulse h-28" />
        ) : (
        ...
        <div className="space-y-3">
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {[1, 2, 3, 4, 5, 6].map((n) => (
                <div key={n} className="p-5 rounded-2xl bg-white/5 border border-white/10 animate-pulse h-32" />
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {modules.movies && (
                <Link href="/sub-admin/movies" className="p-5 rounded-2xl bg-white/5 hover:bg-cyan-500/10 border border-white/10 hover:border-cyan-500/30 transition group">
                  <div className="flex items-center justify-between mb-2">
                    <MovieIcon className="w-6 h-6 text-cyan-400" />
              ...
              {modules.posters && (
                <Link href="/sub-admin/posters" className="p-5 rounded-2xl bg-white/5 hover:bg-cyan-500/10 border border-white/10 hover:border-cyan-500/30 transition group">
                  <div className="flex items-center justify-between mb-2">
                    <PosterIcon className="w-6 h-6 text-cyan-400" />
                    <span className="text-xs font-bold text-cyan-400 group-hover:translate-x-1 transition">Access →</span>
                  </div>
              ...
              <Link href="/sub-admin/attendance" className="p-5 rounded-2xl bg-white/5 hover:bg-cyan-500/10 border border-white/10 hover:border-cyan-500/30 transition group">
                <div className="flex items-center justify-between mb-2">
                  <CalendarIcon className="w-6 h-6 text-cyan-400" />
                  <span className="text-xs font-bold text-cyan-400 group-hover:translate-x-1 transition">Access →</span>
                </div>
                <h3 className="text-sm font-bold text-white">Attendance & Leaves</h3>
                <p className="text-xs text-gray-400 mt-1">Track attendance, apply leaves, and regularize.</p>
              </Link>
              ...
              {modules.drafts && (
                <Link href="/sub-admin/drafts" className="p-5 rounded-2xl bg-white/5 hover:bg-cyan-500/10 border border-white/10 hover:border-cyan-500/30 transition group">
                  <div className="flex items-center justify-between mb-2">
                    <PencilIcon className="w-6 h-6 text-cyan-400" />
                    <span className="text-xs font-bold text-cyan-400 group-hover:translate-x-1 transition">Access →</span>
                  </div>
