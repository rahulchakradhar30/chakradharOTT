  ...
  const saveDraftManually = useCallback(async () => {
    ...
    if (!emailToUse && typeof window !== "undefined") {
      try {
        ...
        console.warn("Draft session lookup failed:", e);
      }
    }

    if (!emailToUse) {
      emailToUse = "admin@chakradharstream.com";
    }

    if (!type) {
      console.warn("Manual save failed: Missing draft type.");
      return null;
    }

    try {
      setSaving(true);
      const id = await saveDraft(type, formData, emailToUse, draftIdRef.current);
      draftIdRef.current = id;
      setDraftId(id);
