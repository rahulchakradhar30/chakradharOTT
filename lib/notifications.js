...
export async function notifyAdmins({ title, message, type, link }) {
  try {
    const adminsSnap = await adminDb.collection("admins").where("status", "==", "active").get();
    
    const promises = adminsSnap.docs.map(async (adminDoc) => {
    ...
    // Also notify env super-admins (they may not have docs in admins collection)
    const envAdmins = getAllowedAdminEmailsInternal();
    for (const envEmail of envAdmins) {
      // Check if already covered
      const exists = adminsSnap.docs.some((d) => d.id === envEmail);
      if (!exists) {
        const notifRef = adminDb
          .collection("admins")
          .doc(envEmail)
    ...
    await Promise.all(promises);
    console.log(`[NOTIFY] Admin notification sent: "${title}"`);
  } catch (error) {
