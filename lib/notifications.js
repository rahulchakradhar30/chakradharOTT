...
/* ── Notify a single admin by in-app notification & optional email ── */
export async function notifyAdmin(email, { title, message, type, link, sendEmail = true }) {
  try {
    ...
    console.log(`[NOTIFY] Admin in-app notification saved for ${normalizedEmail}: "${title}"`);
    ...
    if (sendEmail) {
      try {
        await sendMail({
          to: normalizedEmail,
          subject: `[Admin Alert] ${title}`,
          text: `Hello,\n\n${message}\n\nView details: ${process.env.NEXT_PUBLIC_APP_URL || "https://chakradharstream.vercel.app"}${link || "/admin/mail"}`,
          html: `
            <div style="background-color: #0c1328; padding: 40px 10px; font-family: sans-serif; color: #f3f4f6;">
              <div style="max-width: 600px; margin: 0 auto; background-color: #060b19; border: 1px solid rgba(6,182,212,0.3); border-radius: 20px; overflow: hidden; box-shadow: 0 8px 30px rgba(0,0,0,0.5);">
                <div style="background: linear-gradient(135deg, #06b6d4 0%, #3b82f6 100%); padding: 25px 20px; text-align: center;">
                  <h1 style="margin: 0; color: #ffffff; font-size: 22px; font-weight: 900;">Chakradhar Stream Alert</h1>
                  <p style="margin: 4px 0 0 0; color: rgba(255,255,255,0.85); font-size: 12px; text-transform: uppercase;">Direct Notification</p>
                </div>
                <div style="padding: 30px;">
                  <h2 style="font-size: 18px; font-weight: bold; margin-top: 0; color: #ffffff;">${title}</h2>
                  <p style="font-size: 14px; color: #d1d5db; line-height: 1.6; margin-bottom: 25px;">${message}</p>
                  ${link ? `
                  <div style="text-align: center;">
                    <a href="${process.env.NEXT_PUBLIC_APP_URL || "https://chakradharstream.vercel.app"}${link}" target="_blank" style="background: linear-gradient(135deg, #06b6d4, #3b82f6); color: #ffffff; text-decoration: none; padding: 12px 28px; border-radius: 30px; font-weight: bold; font-size: 13px; display: inline-block;">
                      Open Details
                    </a>
                  </div>
                  ` : ""}
        ...
        console.log(`[NOTIFY] Alert email dispatched to ${normalizedEmail}`);
      } catch (mailErr) {
        console.warn(`[NOTIFY] Email dispatch to ${normalizedEmail} failed:`, mailErr?.message || mailErr);
      }
    }
  } catch (error) {
