 ...
 */
export async function getContentByGenre(genre) {
  try {
    const moviesSnapshot = await getDocs(
      query(collection(db, "movies"), where("genre", "==", genre), limit(50))
    );

    const premiersSnapshot = await getDocs(
      query(collection(db, "premieres"), where("genre", "==", genre), limit(50))
    );
